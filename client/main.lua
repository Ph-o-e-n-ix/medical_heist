...
Citizen.CreateThread(function()
    while true do
        ...
        if alldead and not nachricht and step3done then 
            nachricht = true 
            Config.MSG(Translation[Config.Locale]['all_eleminated'])
        end
    end
end)
    ...
    while true do
        ...
        if blip1active then 
            ...
            if distance < 50 then 
                RemoveBlip(blip1)
                blip1active = false 
                Config.MSG(Translation[Config.Locale]['eleminate_all'])
        ...
        if object1active and step3done then 
            ...
            if distance < 30 then  
                ...
                if distance < 3 and alldead then
                    DrawText3D(enemylocation.propcoords.x, enemylocation.propcoords.y, enemylocation.propcoords.z, Translation[Config.Locale]['press_e_to_loot'])
                    if IsControlJustReleased(0, 38) then 
                        FreezeEntityPosition(PlayerPedId(), true)
                        startanim(PlayerPedId(), 'rcmextreme3', 'idle')
                        alldead = false 
                        object1active = false
                        Config.Progressbar(Translation[Config.Locale]['loot_in_progress'], 5000)
        ...
        if hackerobjectactive and step1done then 
            ...
            if dst < 25 and not inhack then
                ...
                if dst < 1.5 and not inhack then
                    DrawText3D(randomcoords.x, randomcoords.y, randomcoords.z, Translation[Config.Locale]['press_e_to_hack'])
                    if IsControlJustReleased(0, 38) then 
                        if Config.Framework == 'ESX' then
